...
Процедура ПрочитатьНастройкиСинхронизацииДанных(Отказ)
	...
	Попытка
		...
		Для Каждого НастройкаИзМенеджераСервиса Из НастройкиСинхронизацииИзМенеджераСервиса Цикл
			...
			// Убираем строки для настройки видов обмена, для которых есть замещающий вид обмена.
			Если НастройкаИзМенеджераСервиса.СинхронизацияНастроена = Null
				Или Не НастройкаИзМенеджераСервиса.СинхронизацияНастроена Тогда
				
				Попытка
					ЗамещающийПланОбмена = ОбменДаннымиСервер.ЗначениеНастройкиПланаОбмена(
						НастройкаИзМенеджераСервиса.ПланОбмена,
						"ИмяПланаОбменаДляПереходаНаНовыйОбмен");
				Исключение
					ЗамещающийПланОбмена = Неопределено;
				КонецПопытки;
					
				Если ЗначениеЗаполнено(ЗамещающийПланОбмена) Тогда
					Продолжить;
				КонецЕсли;
