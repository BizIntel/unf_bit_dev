...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Удаление лишних символов из пути Linux для томов хранения файлов.'");
...
Процедура ОбновитьПутьТомаLinux(Параметры) Экспорт
	...
	Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедуре ОбновитьПутьТомаLinux не удалось обработать некоторые тома хранения файлов (пропущены): %1'"), 
			ПроблемныхОбъектов);
		...
		Иначе
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедура ОбновитьПутьТомаLinux обработала очередную порцию томов хранения файлов: %1'"),
			ОбъектовОбработано));
		КонецЕсли;
